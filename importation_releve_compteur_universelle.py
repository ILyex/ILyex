...
Desktop-ready tool for meter-reading import/normalization with:
- Web dashboard (served locally)
- CLI batch mode
- CSV/JSON/XLSX/EXL import
- CSV/XLSX export
"""
...
class ValidationError(ValueError):
    """Validation error for import normalization."""
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if file_path.name == "index.html":
            injected = json.dumps(DEFAULT_MAPPING, ensure_ascii=False, indent=2).encode("utf-8")
            data = data.replace(b"__DEFAULT_MAPPING__", injected)
    ...
    def _handle_import(self, body: bytes) -> None:
        try:
            ...
            if ext == "csv":
                ...
                rows = jpayload.get("readings", []) if isinstance(jpayload, dict) else jpayload
            else:
    ...
    def _handle_export(self, body: bytes) -> None:
        try:
            ...
            if fmt == "csv":
                data = _write_csv_bytes(rows)
                filename = "universal_readings.csv"
            elif fmt in {"xlsx", "exl"}:
                data = _write_xlsx_bytes(rows)
                filename = "universal_readings.xlsx"
            else:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--format", choices=["csv", "json", "xlsx", "exl"])
...
def main() -> None:
    ...
    # Desktop default behavior for double-clicked EXE (no flags).
    if len(sys.argv) == 1:
